...
class UploadOrder extends Action
{
    ...
    {
        if ($this->_request->isPost() && isset($_FILES['csv_file_order']['name']) && $_FILES['csv_file_order']['name']) {
            try {
                // Get the upload path
                ...
                $uploader = $this->fileUploaderFactory->create(['fileId' => 'csv_file_order']);
                ...
                // Success message
                $this->messageManager->addSuccessMessage(__('Order file uploaded and processed successfully.'));
                return $this->resultFactory->create(ResultFactory::TYPE_REDIRECT)->setPath('chatbot/import/order');
            } catch (\Exception $e) {
