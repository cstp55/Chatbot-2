...
class UploadReview extends Action
{
    ...
    {
        if ($this->_request->isPost() && isset($_FILES['csv_file_review']['name']) && $_FILES['csv_file_review']['name']) {
            try {
                // Get the upload path
                ...
                $uploader = $this->fileUploaderFactory->create(['fileId' => 'csv_file_review']);
                ...
                // Success message
                $this->messageManager->addSuccessMessage(__('Review file uploaded and processed successfully.'));
                return $this->resultFactory->create(ResultFactory::TYPE_REDIRECT)->setPath('chatbot/import/review');
            } catch (\Exception $e) {
